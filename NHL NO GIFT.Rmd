...
```{r}
#format example urls
dummy_leg_calc_url <- dummy_leg_calc |> rowwise() |> mutate(url = paste0("https://moneypuck.com/g.htm?id=", season, 0, game_id, collapse = ""))
#show examples
head(dummy_leg_calc_url |> arrange(desc(second_ten_goals)) |> select(-game_id))
```
